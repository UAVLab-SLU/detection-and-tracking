...
from olympe.messages.ardrone3.PilotingState import FlyingStateChanged, SpeedChanged, moveByChanged, moveToChanged, GpsLocationChanged
...
class StreamingExample:
    ...
    def make_hover(self):
        ...
        hover_point_lat = self.drone.get_state(GpsLocationChanged)['latitude']
        hover_point_lon = self.drone.get_state(GpsLocationChanged)['longitude']
        destination_lat = self.drone.get_state(moveToChanged)['latitude']
        destination_lon = self.drone.get_state(moveToChanged)['longitude']
        print("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk")
        self.drone(moveBy(0, 10, 0, 0, _timeout=100)).wait().success()
        # self.drone(moveBy(0, -10, 0, 0, _timeout=100)).wait().success()
        print("Llllllllllllllllllllllllllllllllllllllllllllllllll")
        
        self.drone(                                                      
                moveTo(hover_point_lat, hover_point_lon, 65.0, mode.orientation_mode.none, 0.0,_timeout = 100)                                      
                >> moveToChanged(status=status.DONE, _timeout=100)
                ).wait().success()
        print("moved_backkkkkkkkkkkkkkkkkkkkkkkkkkk")
        self.drone(                                                      
                moveTo(destination_lat, destination_lon, 65.0, mode.orientation_mode.to_target, 0.0,_timeout = 100)                                      
                >> moveToChanged(status=status.DONE, _timeout=100)
                ).wait().success()
    ...
    def returnHomeandLand(self):
        ...
        self.drone(                                                      
            moveTo(self.drone.get_state(HomeChanged)['latitude'], self.drone.get_state(HomeChanged)['longitude'], 65.0, mode.orientation_mode.to_target, 0.0, _timeout = 50)                                      
            >> moveToChanged(status=status.DONE, _timeout=50)
            ).wait().success()
        print(self.drone.get_state(moveToChanged))
        print("Landing...")
    ...
    def fly(self):
        ...
        self.drone(MaxTilt(5.0,_timeout = 100)).wait()
        # print(self.drone.get_state(MaxTiltChanged))
        # print(self.drone.get_state(SpeedChanged))
        # self.drone(extended_move_by(0, 0, -45, 0,25,25,30.0, _timeout=100)).wait().success()
        # self.drone(extended_move_by(70, 0, 0, 0,25,25,30.0, _timeout=100)).wait().success()
        # self.drone(extended_move_by(-70, 0, 0, 0,25,25,30.0, _timeout=100)).wait().success()
        # self.drone(extended_move_by(0, 0, 45, 0,25,25,30.0, _timeout=100)).wait().success()
